...
def starting_invalid_feedback(evaluation):
    ...
    fb = '''\nIn the first step, the molecule in the molecule set should be the target molecule'''.format(evaluation['target_smi'])
...
def reaction_unavailable_feedback(evaluation):
    ...
    fb = '''\nThe reaction {} does not exist in the USPTO dataset.\n'''.format(reaction)
...
def verify_reaction_step(molecule_set, updated_molecule_set, reaction, product, reactants):
    ...
    try:
        ...
        #reaction_outputs = reaction.RunReactants(products_expected)
        reactants_generated = [reactant for reactant in reaction_outputs[0]]

        # Verify if the expected products are among the generated products
    ...
    try:
        ...
        if None in updated_molecule_set or None in original_molecule_set or None in reactants:
