...
def extract_left_molecule(paths):
    left_molecules = []
    for path in paths:
        left_molecule = path.split('>>')[0]
        left_molecules.append(left_molecule)
    return left_molecules


def generate_product_lists(paths):
    product_lists = []
    for i in range(len(paths)):
        product_list = paths[i:]
...
def main():
    ...
    parser.add_argument('--dataset_name', default='USPTO-easy')
    parser.add_argument('--config_default', default='hparams_default.yaml')
    parser.add_argument('--output_dir', type=str, default=None)
    parser.add_argument('--max_oracle_calls', type=int, default=300)
        ...
        for seed in args.seed:
            for i in range(len(data)):
                if i == 0:
                    continue
                print('seed', seed)
                print(f'Optimizing oracle function: {data[i]}')
                optimizer = Optimizer(args=args)
                optimizer.optimize(data[i], route_list, all_fps, config=config_default, seed=seed)
